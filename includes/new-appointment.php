...
if ($is_edit) {
  ...
  wp_die(__('You can only edit existing appointments. Creating new appointments is not allowed.', 'textdomain'));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['wdb_save_appointment'])) {
  ...
  $dietitian_note = isset($_POST['dietitian_note']) ? sanitize_textarea_field($_POST['dietitian_note']) : '';
  ...
  if ($is_edit) {
    ...
    if ($is_dietitian) {
      ...
      if (!empty($dietitian_note) && !empty($appointment->order_id)) {

        update_post_meta($appointment->order_id, 'dietitian_note', $dietitian_note);
      }
    } else {
    ...
    if (!empty($dietitian_note) && $order_id) {

      update_post_meta($order_id, 'dietitian_note', $dietitian_note);
    }
  ...
  <form method="POST">
    ...
    <table class="form-table">
      ...
      <tr>
        ...
        <th><label for="dietitian_note">Dietitian Note</label></th>

        <td>

          <textarea name="dietitian_note" id="dietitian_note" class="regular-text" rows="4"><?php echo esc_textarea(get_post_meta($appointment->order_id ?? 0, 'dietitian_note', true)); ?></textarea>
