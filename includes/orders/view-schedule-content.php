...
?>
        ...
        foreach ($grouped as $group_meals):
          foreach ($group_meals as $meal):
            ...
            for ($i = 0; $i < $count; $i++):
              ...
              echo '<td>';
              echo esc_html(ucfirst($meal_types[$i] ?? 'N/A'));
              echo '</td>';
  ...
  <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="updateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form id="rescheduleMealForm" method="POST">
      ...
      function validateDate(selectedDateStr, minDateStr, weekdayOutputSelector, inputElement) {
        ...
        if (selectedDate < minDate) {
          ...
          if (weekdayOutputSelector) {
            ...
            $("#new-weekday-hidden").val(weekdayName);
          }
        }
      }


      function updateWeekdayFromDateInput(dateStr, outputId) {
        if (!dateStr) return;
